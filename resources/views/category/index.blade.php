      ...
      <div class="relative group w-96 mb-4">
        <div class="icons flex justify-between items-start p-5 group-hover:opacity-100 opacity-0 transition-opacity duration-300 absolute top-0 left-0 right-0 z-30">
          ...
          <form action="{{ route('category.destroy', $category->id) }}" method="POST" id="delete-form-{{ $category->id }}">
            @csrf
            @method('DELETE')
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 text-primary cursor-pointer" onclick="document.getElementById('delete-form-{{ $category->id }}').submit();">
