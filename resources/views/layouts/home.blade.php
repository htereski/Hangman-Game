<x-guest-layout>
  <div class="flex flex-col items-center select-none">
    <div class="top-menu flex justify-between items-center px-4 w-[20%]">
      <div class="left">
        <svg xmlns="http://www.w3.org/2000/svg" onclick="settingsModal.showModal()" fill="none" viewBox="0 0 24 24"
          stroke-width="1.5" stroke="currentColor" class="stroke-2 size-6 cursor-pointer text-primary">
      ...
      <div class="modal-box flex flex-col items-center justify-center">
        <div class="dropdown mb-72 flex flex-col items-center justify-center">
          ...
          <ul tabindex="0" class="dropdown-content bg-base-300 rounded-box z-[1] w-52 p-2 shadow-2xl top-[100%] ring-offset-0">
            <li>
              <input type="radio" name="theme-dropdown" class="theme-controller btn btn-sm btn-block btn-ghost text-center"
                aria-label="Escuro" value="default" />
            </li>
            <li>
              <input type="radio" name="theme-dropdown" class="theme-controller btn btn-sm btn-block btn-ghost text-center"
                aria-label="Claro" value="emerald" />
            </li>
          </ul>
        </div>
  ...
  <ul class="menu rounded-box w-56 text-4xl gap-10 flex flex-row justify-center items-center text-primary">
    ...
    <li>
      <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" {{ route('logout')}} onclick="event.preventDefault();
